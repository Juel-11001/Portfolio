...
class SkillItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'skill_percent'=>'required|numeric|max:100'
        ]);
        $skillItem= new SkillItem();
        $skillItem->name=$request->name;
        $skillItem->skill_percent=$request->skill_percent;
        $skillItem->save();
        toastr('Skill Created Successfully!');
    ...
    public function update(Request $request, string $id)
    {
        ...
        $skill->save();
        toastr('Skill Updated Successfully!', 'success');
        return redirect()->route('admin.skill-item.index');
    }
